...
namespace ME.BECS.Extensions.GraphProcessor
{
	...
	public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		{
			scg::List<(BaseNode, SerializableEdge)> disconnectEvents = new scg::List<(BaseNode, SerializableEdge)>();
...
#pragma warning restore CS0618
		...
		public void UpdateExposedParameter(string guid, object value)
		{
			...
			if (value != null && !param.GetValueType().IsAssignableFrom(value.GetType()))
				throw new System.Exception("Type mismatch when updating parameter " + param.name + ": from " + param.GetValueType() + " to " + value.GetType().AssemblyQualifiedName);
		...
		{
			scg::Stack<BaseNode> dfs = new scg::Stack<BaseNode>();
