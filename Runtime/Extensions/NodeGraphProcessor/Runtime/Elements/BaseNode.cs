...
namespace ME.BECS.Extensions.GraphProcessor
{
	...
	public delegate IEnumerable< PortData > CustomPortBehaviorDelegate(scg::List< SerializableEdge > edges);
	...
	public abstract class BaseNode {
		...
		public virtual string style => string.Empty;
		...
		struct PortUpdate
		{
			public scg::List<string>	fieldNames;
			public BaseNode		node;

			public void Deconstruct(out scg::List<string> fieldNames, out BaseNode node)
			{
		...
		public bool IsGroupEnabled() {
			...
			var guid = this.groupGUID;
			if (string.IsNullOrEmpty(guid) == true) return true;

			var group = this.graph.groups.Find(x => x.GUID == guid);
			return group != null ? group.enabled : false;
		...
		public bool UpdatePortsForFieldLocal(string fieldName, bool sendPortUpdatedEvent = true)
		{
			...
			scg::List< string > finalPorts = new scg::List< string >();
		...
		public bool UpdatePortsForField(string fieldName, bool sendPortUpdatedEvent = true)
		{
			...
			fieldsToUpdate.Push(new PortUpdate{fieldNames = new scg::List<string>(){fieldName}, node = this});
		...
		internal void DisableInternal()
		{
			...
			ExceptionToLog.Call((x) => x.Disable(), this);
		}

		internal void DestroyInternal() => ExceptionToLog.Call((x) => x.Destroy(), this);
		...
		public void OnProcess()
		{
			...
			ExceptionToLog.Call((x) => x.Process(), this);
		...
		{
			scg::Stack<BaseNode> dependencies = new scg::Stack<BaseNode>();
