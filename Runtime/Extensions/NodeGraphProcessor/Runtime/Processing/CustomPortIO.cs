...
namespace ME.BECS.Extensions.GraphProcessor
{
	...
	public delegate void CustomPortIODelegate(BaseNode node, scg::List< SerializableEdge > edges, NodePort outputPort = null);
	...
	public static class CustomPortIO
	{
		...
		static Dictionary< Type, scg::List< Type > >	assignableTypes = new Dictionary< Type, scg::List< Type > >();
		...
		static void LoadCustomPortMethods()
		{
			...
			foreach (var type in AppDomain.CurrentDomain.GetAllTypes())
			{
				...
				foreach (var method in methods)
				{
					...
					var p2 = Expression.Parameter(typeof(scg::List< SerializableEdge >), "edges");
		...
		static void AddAssignableTypes(Type fromType, Type toType)
		{
			if (!assignableTypes.ContainsKey(fromType))
				assignableTypes[fromType] = new scg::List< Type >();
