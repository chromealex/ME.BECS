namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryScheduleExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this QueryBuilder builder, in T job) where T : struct, IJobComponents<T0,T1,T2,T3,T4> where T0 : unmanaged, IComponent where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent {
        ...
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this Query staticQuery, in T job, in SystemContext context) where T : struct, IJobComponents<T0,T1,T2,T3,T4> where T0 : unmanaged, IComponent where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent {
            return staticQuery.Schedule<T, T0,T1,T2,T3,T4>(in job, in context.world, context.dependsOn);
        }
        
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this Query staticQuery, in T job, in World world, JobHandle dependsOn = default) where T : struct, IJobComponents<T0,T1,T2,T3,T4> where T0 : unmanaged, IComponent where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent {
            var state = world.state;
            var query = API.MakeStaticQuery(QueryContext.Create(state, world.id), dependsOn).FromQueryData(state, world.id, state->queries.GetPtr(state, staticQuery.id));
            return query.Schedule<T, T0,T1,T2,T3,T4>(in job);
        }

        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobComponents<T0,T1,T2,T3,T4> where T0 : unmanaged, IComponent where T1 : unmanaged, IComponent where T2 : unmanaged, IComponent where T3 : unmanaged, IComponent where T4 : unmanaged, IComponent {
    ...
    public static unsafe partial class JobComponentsExtensions_1 {
        ...
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this T jobData, in CommandBuffer* buffer, JobHandle dependsOn = default)
            ...
            var data = new JobData<T, T0,T1,T2,T3,T4>() {
                jobData = jobData,
                buffer = buffer,
                c0 = buffer->state->components.GetRW<T0>(buffer->state),c1 = buffer->state->components.GetRW<T1>(buffer->state),c2 = buffer->state->components.GetRW<T2>(buffer->state),c3 = buffer->state->components.GetRW<T3>(buffer->state),c4 = buffer->state->components.GetRW<T4>(buffer->state),
            };
        ...
        internal struct JobProcess<T, T0,T1,T2,T3,T4>
            ...
            private static void Execute(ref JobData<T, T0,T1,T2,T3,T4> jobData, System.IntPtr additionalData, System.IntPtr bufferRangePatchData, ref JobRanges ranges, int jobIndex) {
                ...
                for (int i = 0; i < jobData.buffer->count; ++i) {
                    ...
                    jobData.jobData.Execute(ref jobData.c0.Get(entId),ref jobData.c1.Get(entId),ref jobData.c2.Get(entId),ref jobData.c3.Get(entId),ref jobData.c4.Get(entId));
                }
