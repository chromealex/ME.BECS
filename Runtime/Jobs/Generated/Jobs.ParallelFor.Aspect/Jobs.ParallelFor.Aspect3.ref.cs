namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryAspectParallelScheduleExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2>(this QueryBuilder builder, in T job = default) where T : struct, IJobParallelForAspects<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
            ...
            builder.builderDependsOn = job.Schedule<T, T0,T1,T2>(builder.commandBuffer.ptr, builder.parallelForBatch, builder.isUnsafe, builder.builderDependsOn);
        ...
        public static JobHandle Schedule<T, T0,T1,T2>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobParallelForAspects<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
            staticQuery.builderDependsOn = job.Schedule<T, T0,T1,T2>(staticQuery.commandBuffer.ptr, staticQuery.parallelForBatch, staticQuery.isUnsafe, staticQuery.builderDependsOn);
    ...
    public static unsafe partial class JobParallelForAspectExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2>(this T jobData, CommandBuffer* buffer, uint innerLoopBatchCount, bool unsafeMode, JobHandle dependsOn = default)
            ...
            var dataVal = new JobData<T, T0,T1,T2>() {
                ...
                jobInfo = jobInfo,
                buffer = buffer,
                a0 = WorldAspectStorage.Initialize<T0>(buffer->worldId),a1 = WorldAspectStorage.Initialize<T1>(buffer->worldId),a2 = WorldAspectStorage.Initialize<T2>(buffer->worldId),
            };
