    ...
    public static unsafe partial class JobParallelForAspectExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2>(this T jobData, in CommandBuffer* buffer, uint innerLoopBatchCount, bool unsafeMode, JobHandle dependsOn = default)
            ...
            var dataVal = new JobData<T, T0,T1,T2>() {
                ...
                jobData = jobData,
                buffer = buffer,
                a0 = buffer->state->aspectsStorage.Initialize<T0>(buffer->state),a1 = buffer->state->aspectsStorage.Initialize<T1>(buffer->state),a2 = buffer->state->aspectsStorage.Initialize<T2>(buffer->state),
            };
