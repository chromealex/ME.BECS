    ...
    public static unsafe partial class JobParallelForAspectExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4>(this T jobData, CommandBuffer* buffer, uint innerLoopBatchCount, bool unsafeMode, JobHandle dependsOn = default)
            ...
            var dataVal = new JobData<T, T0,T1,T2,T3,T4>() {
                ...
                jobInfo = jobInfo,
                buffer = buffer,
                a0 = WorldAspectStorage.Initialize<T0>(buffer->worldId),a1 = WorldAspectStorage.Initialize<T1>(buffer->worldId),a2 = WorldAspectStorage.Initialize<T2>(buffer->worldId),a3 = WorldAspectStorage.Initialize<T3>(buffer->worldId),a4 = WorldAspectStorage.Initialize<T4>(buffer->worldId),
            };
