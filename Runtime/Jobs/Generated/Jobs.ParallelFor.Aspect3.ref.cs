namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryAspectScheduleExtensions {
        ...
        public static JobHandle ScheduleParallelFor<T, T0,T1,T2>(this QueryBuilder builder, in T job) where T : struct, IJobParallelForAspect<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
        ...
        public static JobHandle ScheduleParallelFor<T, T0,T1,T2>(this Query staticQuery, in T job, in SystemContext context) where T : struct, IJobParallelForAspect<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
            return staticQuery.ScheduleParallelFor<T, T0,T1,T2>(in job, in context.world, context.dependsOn);
        }
        
        public static JobHandle ScheduleParallelFor<T, T0,T1,T2>(this Query staticQuery, in T job, in World world, JobHandle dependsOn = default) where T : struct, IJobParallelForAspect<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
            var state = world.state;
            var query = API.MakeStaticQuery(QueryContext.Create(state, world.id), dependsOn).FromQueryData(state, world.id, state->queries.GetPtr(state, staticQuery.id));
            return query.ScheduleParallelFor<T, T0,T1,T2>(in job);
        }

        public static JobHandle ScheduleParallelFor<T, T0,T1,T2>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobParallelForAspect<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
    ...
    public interface IJobParallelForAspect<T0,T1,T2> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect where T2 : unmanaged, IAspect {
    ...
    public static unsafe partial class JobParallelForAspectExtensions_1 {
        ...
        internal struct JobProcess<T, T0,T1,T2>
            ...
            public static readonly Unity.Burst.SharedStatic<System.IntPtr> jobReflectionData = Unity.Burst.SharedStatic<System.IntPtr>.GetOrCreate<JobProcess<T, T0,T1,T2>>();
            ...
            private static void Execute(ref JobData<T, T0,T1,T2> jobData, System.IntPtr bufferPtr, System.IntPtr bufferRangePatchData, ref JobRanges ranges, int jobIndex) {
                ...
                while (JobsUtility.GetWorkStealingRange(ref ranges, jobIndex, out var begin, out var end) == true) {
                    ...
                    for (uint i = (uint)begin; i < end; ++i) {
                        ...
                        jobData.c0.ent = new Ent(entId, gen, jobData.buffer->worldId);jobData.c1.ent = new Ent(entId, gen, jobData.buffer->worldId);jobData.c2.ent = new Ent(entId, gen, jobData.buffer->worldId);
                        jobData.jobData.Execute(ref jobData.c0,ref jobData.c1,ref jobData.c2);
                    }
