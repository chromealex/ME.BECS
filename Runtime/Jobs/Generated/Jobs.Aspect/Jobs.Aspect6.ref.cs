    ...
    public static unsafe partial class JobAspectExtensions {
        ...
        public static JobHandle Schedule<T, T0,T1,T2,T3,T4,T5>(this T jobData, in CommandBuffer* buffer, bool unsafeMode, uint innerLoopBatchCount, ScheduleMode scheduleMode, JobHandle dependsOn = default)
            ...
            var dataVal = new JobData<T, T0,T1,T2,T3,T4,T5>() {
                ...
                jobData = jobData,
                buffer = buffer,
                a0 = buffer->state->aspectsStorage.Initialize<T0>(buffer->state),a1 = buffer->state->aspectsStorage.Initialize<T1>(buffer->state),a2 = buffer->state->aspectsStorage.Initialize<T2>(buffer->state),a3 = buffer->state->aspectsStorage.Initialize<T3>(buffer->state),a4 = buffer->state->aspectsStorage.Initialize<T4>(buffer->state),a5 = buffer->state->aspectsStorage.Initialize<T5>(buffer->state),
            };
