namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryAspectScheduleExtensions {
        ...
        public static JobHandle Schedule<T, T0>(this QueryBuilder builder, in T job = default) where T : struct, IJobForAspects<T0> where T0 : unmanaged, IAspect {
            ...
            builder.builderDependsOn = job.Schedule<T, T0>(in builder.commandBuffer, builder.isUnsafe, builder.parallelForBatch, builder.scheduleMode, builder.builderDependsOn);
        ...
        public static JobHandle Schedule<T, T0>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobForAspects<T0> where T0 : unmanaged, IAspect {
            staticQuery.builderDependsOn = job.Schedule<T, T0>(in staticQuery.commandBuffer, staticQuery.isUnsafe, staticQuery.parallelForBatch, staticQuery.scheduleMode, staticQuery.builderDependsOn);
    ...
    public static unsafe partial class JobAspectExtensions {
        ...
        public static JobHandle Schedule<T, T0>(this T jobData, in CommandBuffer* buffer, bool unsafeMode, uint innerLoopBatchCount, ScheduleMode scheduleMode, JobHandle dependsOn = default)
            where T0 : unmanaged, IAspect
            ...
            var dataVal = new JobData<T, T0>() {
                ...
                jobData = jobData,
                buffer = buffer,
                a0 = buffer->state->aspectsStorage.Initialize<T0>(buffer->state),
            };
