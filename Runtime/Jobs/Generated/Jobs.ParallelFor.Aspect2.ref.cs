namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryAspectScheduleExtensions {
        ...
        public static JobHandle ScheduleParallelFor<T, T0,T1>(this Query staticQuery, in T job, in SystemContext context) where T : struct, IJobParallelForAspect<T0,T1> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect {
            return staticQuery.ScheduleParallelFor<T, T0,T1>(in job, in context.world, context.dependsOn);
        }
        
        public static JobHandle ScheduleParallelFor<T, T0,T1>(this Query staticQuery, in T job, in World world, JobHandle dependsOn = default) where T : struct, IJobParallelForAspect<T0,T1> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect {
            var state = world.state;
            var query = API.MakeStaticQuery(QueryContext.Create(state, world.id), dependsOn).FromQueryData(state, world.id, state->queries.GetPtr(state, staticQuery.id));
            return query.ScheduleParallelFor<T, T0,T1>(in job);
        }

        public static JobHandle ScheduleParallelFor<T, T0,T1>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobParallelForAspect<T0,T1> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect {
    ...
    public interface IJobParallelForAspect<T0,T1> where T0 : unmanaged, IAspect where T1 : unmanaged, IAspect {
    ...
    public static unsafe partial class JobParallelForAspectExtensions_1 {
        ...
        public static JobHandle ScheduleParallelFor<T, T0,T1>(this T jobData, in CommandBuffer* buffer, uint innerLoopBatchCount, JobHandle dependsOn = default)
            ...
            var parameters = new JobsUtility.JobScheduleParameters(UnsafeUtility.AddressOf(ref data), GetReflectionData<T, T0,T1>(), dependsOn, ScheduleMode.Parallel);
        ...
        internal struct JobProcess<T, T0,T1>
            ...
            public static readonly Unity.Burst.SharedStatic<System.IntPtr> jobReflectionData = Unity.Burst.SharedStatic<System.IntPtr>.GetOrCreate<JobProcess<T, T0,T1>>();
