namespace ME.BECS.Jobs {
    ...
    public static unsafe partial class QueryAspectsComponentsScheduleExtensions1_1 {
        ...
        public static JobHandle Schedule<T, A0, C0>(this QueryBuilder builder, in T job = default) where T : struct, IJobFor1Aspects1Components<A0, C0> where A0 : unmanaged, IAspect where C0 : unmanaged, IComponentBase {
            ...
            builder.builderDependsOn = job.Schedule<T, A0, C0>(builder.commandBuffer.ptr, builder.isUnsafe, builder.isReadonly, builder.parallelForBatch, builder.scheduleMode, builder.builderDependsOn);
        ...
        public static JobHandle Schedule<T, A0, C0>(this QueryBuilderDisposable staticQuery, in T job) where T : struct, IJobFor1Aspects1Components<A0, C0> where A0 : unmanaged, IAspect where C0 : unmanaged, IComponentBase {
            staticQuery.builderDependsOn = job.Schedule<T, A0, C0>(staticQuery.commandBuffer.ptr, staticQuery.isUnsafe, staticQuery.isReadonly, staticQuery.parallelForBatch, staticQuery.scheduleMode, staticQuery.builderDependsOn);
    ...
    public static unsafe partial class JobAspectsComponentsExtensions1_1 {
        ...
        [CodeGeneratorIgnore]
        public static JobHandle Schedule<T, A0, C0>(this T jobData, CommandBuffer* buffer, bool unsafeMode, bool isReadonly, uint innerLoopBatchCount, ScheduleMode scheduleMode, JobHandle dependsOn = default)
            where A0 : unmanaged, IAspect
            where C0 : unmanaged, IComponentBase
                    ...
                    if ((jobData.scheduleFlags & ScheduleFlags.Parallel) != 0) {
                        while (JobsUtility.GetWorkStealingRange(ref ranges, jobIndex, out var begin, out var end) == true) {
                            ...
                            for (uint i = (uint)begin; i < end; ++i) {
                                ...
                                aspect0.ent = ent;
                                jobData.jobData.Execute(in jobInfo, in ent, ref aspect0, ref jobData.c0.GetReadonly(ent.id, ent.gen));
                            }
