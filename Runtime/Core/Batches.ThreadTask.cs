    ...
    public unsafe partial struct Batches {
            ...
            public void Add(in Task task) {
                ...
                if (this.root == null) {
                    this.root = _make(new Node() {
                        list = new UnsafeList<Task>(THREAD_TASK_LIST_CAPACITY, Unity.Collections.Allocator.Persistent),
                    });
                    this.head = this.root;
        ...
        private void ApplyTasks(State* state, ushort worldId, int count, int createDestroyCount, ref int readCount, ref ThreadTaskList.Node* rover, ref int maxCount) {
            ...
            if (createDestroyCount > 0) {
                ...
                var maxId = 0u;
                var createEntities = UnsafeList<Ent>.Create(count, Unity.Collections.Allocator.Temp);
                var destroyEntities = UnsafeList<uint>.Create(count, Unity.Collections.Allocator.Temp);
                var k = kIndex;
