...
namespace ME.BECS {
    ...
    public static class quaternionext {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Unity.Mathematics.quaternion ClampAngleOnAxis(this Unity.Mathematics.quaternion rot, int axis, float minAngle, float maxAngle) {
            var angles = Unity.Mathematics.math.Euler(rot);
            angles[axis] = Unity.Mathematics.math.clamp(angles[axis], minAngle, maxAngle);
            return Unity.Mathematics.quaternion.Euler(Unity.Mathematics.math.radians(angles));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ME.BECS.FixedPoint.quaternion ClampAngleOnAxis(this ME.BECS.FixedPoint.quaternion rot, int axis, sfloat minAngle, sfloat maxAngle) {
            var angles = rot.ToEuler();
            angles[axis] = ME.BECS.FixedPoint.math.clamp(angles[axis], minAngle, maxAngle);
            return ME.BECS.FixedPoint.quaternion.Euler(ME.BECS.FixedPoint.math.radians(angles));
        }
...
namespace ME.BECS.FixedPoint
{
    public static partial class math
    {
        ...
        public static float3 EulerXYZ(quaternion q)
        {
            ...
            var d3 = qv * qv;
            var euler = FixedPoint.float3.zero;
