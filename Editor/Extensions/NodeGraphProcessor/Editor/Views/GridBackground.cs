...
namespace ME.BECS.Extensions.GraphProcessor
{
  ...
  public class GridBackground : ImmediateModeElement
  {
    ...
    private float spacing => this.m_Spacing;
    ...
    protected override void ImmediateRepaint()
    {
      ...
      for (vector3_2.y = vector3_3.y = (float) ((double) y % ((double) num2 * (double) vector3_1.y) - (double) num2 * (double) vector3_1.y); (double) vector3_2.y < (double) layout1.height + (double) num2; vector3_3.y += this.spacing * vector3_1.y * (float) this.thickLines)
      {
        GL.Begin(GL.LINES);
        GL.Color(this.thickLineColor);
        GL.Vertex(this.Clip(layout1, vector3_2));
