...
namespace ME.BECS.Extensions.GraphProcessor {
	...
	public class RelayNode : BaseNode {
		...
		public struct PackedRelayData {
			...
			public scg::List<BaseNode> nodes;
			public scg::List<object> values;
			public scg::List<string> names;
			public scg::List<Type> types;
		...
		public void GetInputs(scg::List<SerializableEdge> edges) {
			...
			if (edges.Count == 1 && edges.First().outputNode.GetType() == typeof(RelayNode)) {
				if (edges.First().passThroughBuffer != null) input = (PackedRelayData)edges.First().passThroughBuffer;
			} else {
				input.nodes = edges.Select(x => x.outputNode).ToList();
		...
		public void PushOutputs(scg::List<SerializableEdge> edges, NodePort outputPort) {
		...
		IEnumerable<PortData> InputPortBehavior(scg::List<SerializableEdge> edges) {
		...
		IEnumerable<PortData> OutputPortBehavior(scg::List<SerializableEdge> edges) {
			if (inputPorts.Count == 0) {
		...
		static scg::List<(Type, string)> s_empty = new scg::List<(Type, string)>();

		public scg::List<(Type type, string name)> GetUnderlyingPortDataList() {
			// get input edges:
		...
		public scg::List<SerializableEdge> GetNonRelayEdges() {
			var inputEdges = inputPorts?[0]?.GetEdges();

			// Iterate until we don't have a relay node in input
			while (inputEdges.Count == 1 && inputEdges.First().outputNode.GetType() == typeof(RelayNode)) inputEdges = inputEdges.First().outputNode.inputPorts[0]?.GetEdges();
