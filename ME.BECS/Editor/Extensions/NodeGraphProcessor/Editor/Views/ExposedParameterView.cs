...
namespace ME.BECS.Extensions.GraphProcessor
{
	...
	public class ExposedParameterView : PinnedElementView
	{
        ...
        scg::List<Rect> blackboardLayouts = new scg::List<Rect>();
        ...
        void OnDragUpdatedEvent(DragUpdatedEvent evt)
        {
            ...
            foreach (var obj in graphSelectionDragData as scg::List<ISelectable>)
            {
        ...
        void OnDragPerformEvent(DragPerformEvent evt)
        {
            ...
            int newIndex = GetInsertIndexFromMousePosition(evt.mousePosition);
            foreach (var obj in DragAndDrop.GetGenericData("DragSelection") as scg::List<ISelectable>)
            {
                if (obj is ExposedParameterFieldView view)
                {
                    if (!updateList)
