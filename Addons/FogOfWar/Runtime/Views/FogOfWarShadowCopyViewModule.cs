...
namespace ME.BECS.FogOfWar {
    ...
    public class FogOfWarShadowCopyViewModule : IViewApplyState, IViewInitialize, IViewOnValidate {
        ...
        public void UpdateVisibility(in EntRO ent, bool forced) {
            ...
            if (activePlayer.readTeam != objectForTeam) {
                this.ApplyVisibility(false, forced);
                return;
            }
            var position = ent.GetAspect<TransformAspect>().GetWorldMatrixPosition();
            this.ApplyVisibility(this.fow.IsVisible(in activePlayer, in position) == false && this.fow.IsExplored(in activePlayer, in position) == true, forced);
        }
