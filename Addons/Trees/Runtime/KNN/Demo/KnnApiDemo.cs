...
public static class KnnApiDemo  {
	public static void Demo() {
		...
		var points = new NativeArray<KnnContainer<ME.BECS.Ent>.Node>(100000, Allocator.Persistent);
		var rand = new Random(123456);
		for (int i = 0; i < points.Length; ++i) {
			points[i] = new KnnContainer<ME.BECS.Ent>.Node() {
				position = rand.NextFloat3(),
			};
		}
		...
		var knnContainer = new KnnContainer<ME.BECS.Ent>(points, Allocator.TempJob);
		Profiler.EndSample();
		...
		var result = new NativeArray<ME.BECS.Ent>(kNeighbours, Allocator.TempJob);
		...
		var results = new NativeArray<ME.BECS.Ent>(queryPoints * kNeighbours, Allocator.TempJob);
