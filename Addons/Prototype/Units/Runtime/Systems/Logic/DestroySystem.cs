        ...
        public struct DestroyJob : IJobForAspects<UnitAspect> {
            ...
            public void Execute(in JobInfo jobInfo, in Ent ent, ref UnitAspect unit) {
                if (unit.readHealth <= 0u) {
                    ...
                    EffectUtils.CreateEffect(in jobInfo, tr.position, tr.rotation, unit.ent.ReadStatic<UnitEffectOnDestroyComponent>().effect, unit.readOwner.GetAspect<ME.BECS.Players.PlayerAspect>());
